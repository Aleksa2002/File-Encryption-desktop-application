...
namespace ZastitaInformacija.Core.Algorithms.Hash
{
    internal class TigerHash
    {
        // 4 S-boxes of 256 64-bit entries each.
        // Paste the standard Tiger T1..T4 tables here (ulong[256] each).
        // You can find them in the Tiger reference ("tiger.h") — identical across implementations.
        readonly static ulong[] T1 =
         [
        ...
        // If you prefer Tiger2 (different padding rule), set this to true and change pad byte to 0x01
        private const bool Tiger2Padding = false;
        ...
        public byte[] ComputeHash(byte[] data)
        {
            ...
            // Process 512-bit blocks (64 bytes)
            int full = data.Length & ~63;
            int i = 0;
            while (i < full)
            {
                Compress(data, i, ref a, ref b, ref c);
                i += 64;
            }

            // Pad remaining
            Span<byte> block = stackalloc byte[64];
            int rem = data.Length - full;
            if (rem > 0)
                new ReadOnlySpan<byte>(data, full, rem).CopyTo(block);

            // Padding: append 0x80 (Tiger) or 0x01 (Tiger2), then zeros, then length in bits (128-bit little endian)
            block[rem] = Tiger2Padding ? (byte)0x01 : (byte)0x80;
            // If there's not enough space for 16-byte length, process and clear block
            if (rem >= 56)
            {
                Compress(block, 0, ref a, ref b, ref c);
                block.Clear();
            }

            // Total length in bits (128-bit little endian)
            ulong bitLen = (ulong)data.LongLength * 8UL;
            // low 64 bits:
            BinaryPrimitives.WriteUInt64LittleEndian(block.Slice(56, 8), bitLen);
            // high 64 bits (we don’t exceed 2^64 bits typically):
            block.Slice(64 - 16, 8).Clear();

            Compress(block, 0, ref a, ref b, ref c);

            // Output a || b || c (192 bits)
            byte[] digest = new byte[24];
            BinaryPrimitives.WriteUInt64LittleEndian(digest.AsSpan(0, 8), a);
            BinaryPrimitives.WriteUInt64LittleEndian(digest.AsSpan(8, 8), b);
            BinaryPrimitives.WriteUInt64LittleEndian(digest.AsSpan(16, 8), c);
            return digest;
        }

        private static void Compress(ReadOnlySpan<byte> block, int offset, ref ulong a, ref ulong b, ref ulong c)
        {
            // Read 8 little-endian 64-bit words
            ulong[] x = new ulong[8];
            for (int i = 0; i < 8; i++)
                x[i] = BinaryPrimitives.ReadUInt64LittleEndian(block.Slice(offset + i * 8, 8));

            ulong aa = a, bb = b, cc = c;

            Pass(ref a, ref b, ref c, x, 5UL);
            KeySchedule(x);
            Pass(ref c, ref a, ref b, x, 7UL);
            KeySchedule(x);
            Pass(ref b, ref c, ref a, x, 9UL);
